...
export interface OffsetProgressInfo {
  position: number;
  value: MotionValue<number>; // the global progress
  transitionRange: number;
  activeRange?: number;
  clamp?: boolean;
}
export function useOffsetProgress({
  value,
  position,
  clamp = false,
  transitionRange = 10, // when performing transition
  activeRange = transitionRange / 2, // the stable range in which the item is active
}: OffsetProgressInfo) {
  const offsetProgress = useTransform(
    value,
    [
      position - transitionRange,
      position - activeRange,
      position + activeRange,
      position + transitionRange,
    ],
    [-1, 0, 0, 1],
